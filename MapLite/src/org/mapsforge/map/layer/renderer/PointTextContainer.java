...
class PointTextContainer {
	...
	PointTextContainer(String text, double x, double y, Paint paintFront) {
		...
		Rect rect = new Rect();
		this.paintFront.getTextBounds(text, 0, text.length(), rect);
		
		this.boundary = new Rectangle(0, 0, rect.width(), rect.height());
	}
	...
	PointTextContainer(String text, double x, double y, Paint paintFront, Paint paintBack) {
		...
		Rect rect = new Rect();
		if (paintBack != null) {		
			this.paintBack.getTextBounds(text, 0, text.length(), rect);
		} else {
			this.paintFront.getTextBounds(text, 0, text.length(), rect);
		}
		this.boundary = new Rectangle(0, 0, rect.width(), rect.height());
